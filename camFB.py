...
def gunCameraLoop(cam):
    # FUUUUUUUUUUUUUUUUUCK THE CAMERA I BOUGHT DOESN'T SUPPORT 320X240
    while True:
...
def driveCameraLoop(cam):
    while True:
        ...
        fTime = perf_counter() - fTimeStart
        # print(f"Drive frametime: {fTime:.4f} sec, FPS: {1 / fTime:.2f}", end="; ")
...
def getDriveCamAndInit():
    ...
    os.system(
        f"v4l2-ctl -d /dev/video{driveCamID} --set-ctrl brightness=64 --set-ctrl contrast=32 --set-ctrl saturation=80 --set-ctrl gain=0 --set-ctrl sharpness=6 --set-ctrl backlight_compensation=12 --set-ctrl auto_exposure=3 --set-ctrl exposure_dynamic_framerate=1"
    )
...
def getGunCamAndInit():
    ...
    videoIDs = videoIDs.stdout
    location = videoIDs.find(
        "Inno"
    )  # This is the model of my USB camera. If i use two of the same model, i will have to find a new way to do this
    if location != -1:  # If it's not found (i.e. not plugged in), quit.
        # First, in the string of videoIDs, find first index the first instance of "video" after the index of my ID. Then, add 5 to that index which gives the index of the id following just after "video". Then, since im slicing from a string, convert to an int.
        gunCamID = int(videoIDs[videoIDs.find("video", location) + 5])
    else:
        print("Could not find gun camera! Exiting!")
        exit()
    # FUUUUUUUUUUUUUUUUUCK THE CAMERA I BOUGHT DOESN'T SUPPORT 320X240
    gunCam = cv2.VideoCapture(gunCamID)
    gunCam.set(cv2.CAP_PROP_FRAME_WIDTH, 320)  # Set the width, then the height
    gunCam.set(cv2.CAP_PROP_FRAME_HEIGHT, 240)

    # Brightness, contrast, saturation and such are personal preference. Backlight compensation is good for sillouettes against the sun. Auto exposure and exposure dynamic framerate ONLY work together. They must BOTH be active for auto exposure.
    os.system(
        f"v4l2-ctl -d /dev/video{gunCamID} --set-ctrl brightness=128 --set-ctrl contrast=64 --set-ctrl saturation=80 --set-ctrl gain=0 --set-ctrl sharpness=255 --set-ctrl backlight_compensation=1 --set-ctrl auto_exposure=3 --set-ctrl exposure_dynamic_framerate=1"
    )
...
def main():
    driveCam = getDriveCamAndInit()
    gunCam = getGunCamAndInit()
    ...
    threading.Thread(target=driveCameraLoop, args=(driveCam,), daemon=True).start()
    threading.Thread(target=gunCameraLoop, args=(gunCam,), daemon=True).start()
